...
//            throw new IllegalArgumentException("Cannot add edge with vertices that do not exist");
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence = new LinkedList<>();
            for (T f: copyPath.dataSequence) {
                this.dataSequence.add(f);
            }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        //tries to get the vertices from the hashtable
        if( vertices.get(start)==null||vertices.get(end)==null){
            throw new NoSuchElementException();
        }
        PriorityQueue<Path> sucsessfulpath = new PriorityQueue<>();// makes a Priority queue
        LinkedList<Path> notDone = new LinkedList<>();//makes a list of not done paths
        LinkedList<Path> notWorking = new LinkedList<>();//makes a list of not working paths
        LinkedList<Vertex> visited = new LinkedList<>();//Makes a list of visited vertices
        notDone.add(new Path(vertices.get(start)));// adds the first path to the not done
        while (notDone.size()!= 0 ){//while loop to go through the whole thing until done
            for(int i = 0 ; i <notDone.size();i++){//iterates through all the paths
                Path p = notDone.get(i);// sets a path variable
                //case #1 you get to the end
                if(p.end ==vertices.get(end)){
                    notDone.remove(p);//remove from not done
                    sucsessfulpath.add(p);//add to successful Priority queue
                    break;
                    //Case #2 if it is not the end vertex and there are edges leaving and the end vertex is not in visited
                }else if(!(p.end.equals(vertices.get(end)))&& p.end.edgesLeaving.size()>0&&!visited.contains(p.end) ){
                    LinkedList<Edge> temp = p.end.edgesLeaving;
                    visited.add((p.end));//adds p to visited
                    //adds them all to the not done paths
                    for(Edge e: temp){
                        notDone.add(new Path(p,e));
                    }
                    //removes this current path from notdone
                    notDone.remove(p);
                    break;
                }
                //case#3 if it has been visted and it is not the end
                else if((!p.end.equals(vertices.get(end))) ){
                    notWorking.add(p);
                    notDone.remove(p);
                    break;
                }
        ...
        if(sucsessfulpath.isEmpty()){
            throw new NoSuchElementException("Path cannot be found");
        }
        //polls the shortest one
        return sucsessfulpath.poll();
    }
